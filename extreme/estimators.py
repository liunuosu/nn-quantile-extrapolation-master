...
def random_forest_k(x, n_forests, a=0.05, c=0.9, seed=42):
    """
...
def evt_estimators(n_replications, n_data, distribution, params, metric="median", return_full=False):
    ...
    parameters = str(params).replace(":", "_")
    pathdir = Path("ckpt", distribution, "extrapolation", parameters)
...
def evt_estimators_real(distribution, alpha, trunc, metric="median", return_full=False):
    """
    Evaluation of extreme quantile estimators based on simulated heavy-tailed data
    ...
    try:
        dict_evt = np.load(Path(pathdir, "evt_estimators.npy"), allow_pickle=True)[()]
    except FileNotFoundError:
        ...
        np.save(Path(pathdir, "evt_estimators.npy"), dict_evt)

    if return_full:
        return dict_evt
    df = pd.DataFrame(columns=list_estimators, index=["K", "quantile"])
    for estimator in list_estimators:
