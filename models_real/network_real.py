...
class ExtrapolateNN_real():
    ...
    def train(self, alpha, data_train, X, n_epochs, loss, batch_size, verbose, distribution=None, y=None, real=False, **kwargs):
        ckpt_pathdir = Path("ckpt_real", distribution, "training")
        # init paths
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            if epoch % verbose == 0 and epoch > 10:  # action every verbose epochs
                self.save_checkpoint(epoch, X, y, real, alpha)  # save parameters, losses
    ...
    # Evaluation
    # ======================
    def save_checkpoint(self, epoch, X, y, real, alpha):
        """save parameters and evaluation metrics"""
        self.eval_criterias(epoch, X, y, real, alpha)
    ...
    def eval_criterias(self, epoch, X_order, y, real, alpha, *args, **kwargs):
